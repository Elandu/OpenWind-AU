...
When the environment variable is unset, the default is
`%LOCALAPPDATA%\OpenWind-AU\osm_building_footprints` on Windows when `LOCALAPPDATA` is available,
and `~/.cache/openwind-au/osm_building_footprints` otherwise. Generated filenames have the form
`osm-<sha256>.json`; the digest is derived from the rounded query coordinates and radius so a
directory listing does not directly reveal the location. This is filename obfuscation, not
encryption: each cache payload still contains the query coordinates and derived footprint data.
Treat the cache directory as project-location data, restrict filesystem access, and use encrypted
storage when site locations are sensitive.

OSM cache entries are written to a temporary file, flushed, and installed with an atomic replace.
Reads reject duplicate JSON members, non-finite values, mismatched query identities, corrupt
payloads, and oversized entries. Each entry is limited to 25 MiB; generated entries are pruned to
at most 256 MiB and 512 files, with entries expiring after 30 days. These controls bound the local
outage cache, but they do not provide provider rate limiting or a complete network security
boundary.

To clear saved OSM locations, stop the application and delete the generated `osm-*.json` files,
or remove the dedicated OSM cache directory. The files are derived data and will be recreated
only after another successful provider query. Do not point
`OPENWIND_OSM_FOOTPRINT_CACHE` at a directory containing unrelated files; automatic pruning only
manages filenames generated by OpenWind-AU.
