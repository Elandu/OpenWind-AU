...
def test_generate_standard_terrain_profiles_creates_eight_named_directions() -> None:
    ...
    assert [profile.direction for profile in profiles] == [
        "N",
        "NE",
        "E",
        "SE",
    ...
    assert [profile.azimuth_deg for profile in profiles] == [
        pytest.approx(0),
        pytest.approx(45),
        pytest.approx(90),
        pytest.approx(135),
    ...
    assert all(profile.radius_m == 1000 for profile in profiles)
    assert all(profile.endpoint_latitude for profile in profiles)
    assert all(profile.endpoint_longitude for profile in profiles)
...
def test_run_site_analysis_returns_required_metrics() -> None:
    ...
    if result.features:
        ...
        assert feature.direction in {"N", "NE", "E", "SE", "S", "SW", "W", "NW"}
